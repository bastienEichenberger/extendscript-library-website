...
jsfv['fos_user_profile_form'] = new function() {
    var self = this;
...
// button with span after input
    ...
    this.check_fos_user_profile_form_username = function() {
        var gv;
        result = true;
        result = result && checkError('fos_user_profile_form_username', NotBlank, {message: "fos_user.username.blank"});
        result = result && checkError('fos_user_profile_form_username', UniqueEntity, {entity: "Sdz\\UserBundle\\Entity\\User", identifier_field_id: "fos_user_profile_form_jsfv_identifier", message: "fos_user.username.already_used", service: "doctrine.orm.validator.unique", em: null, repositoryMethod: "findBy", fields: "username", errorPath: null, ignoreNull: true});
        return result;
    };
    this.check_fos_user_profile_form_email = function() {
        var gv;
        result = true;
        result = result && checkError('fos_user_profile_form_email', NotBlank, {message: "fos_user.email.blank"});
        result = result && checkError('fos_user_profile_form_email', Email, {message: "fos_user.email.invalid", checkMX: false, checkHost: false});
        result = result && checkError('fos_user_profile_form_email', UniqueEntity, {entity: "Sdz\\UserBundle\\Entity\\User", identifier_field_id: "fos_user_profile_form_jsfv_identifier", message: "fos_user.email.already_used", service: "doctrine.orm.validator.unique", em: null, repositoryMethod: "findBy", fields: "email", errorPath: null, ignoreNull: true});
        return result;
    };
    this.check_fos_user_profile_form_birthDate = function() {
        var gv;
        result = true;
        result = result && checkError('fos_user_profile_form_birthDate', DateTime, {message: "This value is not a valid datetime."});
        return result;
    };
    this.check_fos_user_profile_form_adresse = function() {
        var gv;
        result = true;
        result = result && checkError('fos_user_profile_form_location_adresse', NotBlank, {message: "fos_user.adresse.blank"});
        result = result && checkError('fos_user_profile_form_location_adresse', IsAdresseValid, {message: "fos_user.adresse.not_valid"});
        return result;
    };
    this.submit = function() {
        var gv, submitForm = true;
        submitForm = self.check_fos_user_profile_form_username() && submitForm;
        submitForm = self.check_fos_user_profile_form_email() && submitForm;
        submitForm = self.check_fos_user_profile_form_birthDate() && submitForm;
        submitForm = self.check_fos_user_profile_form_adresse() && submitForm;
        if (!submitForm) {
            var form = self.id('fos_user_profile_form');
...
// Get the form
            if (form.nodeName.toLowerCase() != 'form') {
                form = self.id('fos_user_profile_form__token').form;
            }
            self.onEvent(form, 'submit', self.submit);
        }
// On blur checks
        self.onEvent('fos_user_profile_form_username', 'blur', self.check_fos_user_profile_form_username);
        self.onEvent('fos_user_profile_form_email', 'blur', self.check_fos_user_profile_form_email);
        self.onEvent('fos_user_profile_form_birthDate', 'blur', self.check_fos_user_profile_form_birthDate);
        self.onEvent('fos_user_profile_form_location_adresse', 'blur', self.check_fos_user_profile_form_adresse);
    };
}
$(jsfv['fos_user_profile_form'].onReady); 